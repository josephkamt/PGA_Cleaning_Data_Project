...
# Merging data sets of test and train into one (STEP 1)
...
# There are duplicate column names in merge_test_train, proceeding to solve this. It gets messy once we get to the merging point. I went ahead of the instructions in this point (STEP3)
...
# Setting descriptive names to activities (STEP4)
merged_labels <- merge(merge_test_train2, activity_labels, by="activitycode", all.x=T)



# Applying aggregate function and generateing final tidy set, exporting to file which will be loaded to github (STEP5)
...
write.table(secTidySet, "./GCDCProject.txt", row.name=F)
